...
export default class AttendanceScene extends Component {
  ...
  _renderBeacons = () => {
    ...
    return this.state.beacons.sort((a, b) => a.accuracy - b.accuracy).map((beacon, ind) => (
      ...
      {this._renderButton('AR 출석 체크 시작', this._stopScanning, '#FF8C00')}
      </View>
    ), this);
  }
  };
...
/*
  _renderEmpty = () => {
    ...
    let text;
    if (!scanning) text = "비콘 감지를 시작하세요!";
    if (scanning && beacons.length) text = "1";
    if (text = "1"){ 
    return (
      <View style={styles.buttonContainer}>
      <Text style={styles.text}>{text}</Text>
          {this._renderButton('AR 출석 체크 시작', this._stopScanning, '#84e2f9')}
      </View>
    );
  }
  else
  {
    text = "아직 비콘이 감지되지 않았습니다.";
  }
  };
  */
  _renderStatusText = () => {
